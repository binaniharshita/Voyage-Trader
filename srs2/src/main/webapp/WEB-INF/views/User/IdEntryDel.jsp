<%@page import="com.danskeit.srs2.bean.ReservationBean"%>
<%@page import="com.danskeit.srs2.*" %>
<%@page import="java.util.*" %>
<%ArrayList<ReservationBean> al=(ArrayList<ReservationBean>)session.getAttribute("Reservations"); %>
<!DOCTYPE html>
<html>
<head>
   ...
   <table id="t01">
        <tr>
                 <th>Reservation ID</th>
                  ...
                  <th>Total Fare</th>
                  <th></th>
                <%for(ReservationBean rb:al)
                { %>
                <tr>
                  <td><%=rb.getReservationid() %></td>
                  <td><%=rb.getSchedules().getScheduleid() %></td>
                  <td><%=rb.getBookingdate() %></td>
                  <td><%=rb.getJourneydate() %></td>
                  <td><%=rb.getNoofseats() %></td>
                  <td><%=rb.getTotalfare() %></td>
                  <td>
                  <form action="delByIdReservation">
                  <button name="reservationid" value="<%=rb.getReservationid() %>" type="submit" class="btn btn--radius-2 btn--blue">Delete</button>
                  </form>
                </td>
                  </tr> 
              <%} %>
