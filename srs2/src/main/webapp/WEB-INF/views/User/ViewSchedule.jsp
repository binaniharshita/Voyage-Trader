<%@page import="com.danskeit.srs2.bean.ScheduleBean"%>
       ...
       <div class="container-login100">
            <div class="card card-4">
                <div class="card-body">
                    ...
                    <form action="viewScheduleDetails">
                    
                        <div class="input-group">
                            <label class="label">Source</label>
                           <!-- <input class="input--style-4" type="text" name="source">-->
                            <select class="input--style-4" name="source">
                            <%for(ScheduleBean sb:al){%>
                            <option value="<%=sb.getRoute().getSource()%>"><%=sb.getRoute().getSource()%></option>
                            <%}%>
                            </select>
                        </div>
                        <div class="input-group">
                            <label class="label">Destination</label>
                            <!--<input class="input--style-4" type="text" name="destination">-->
                             <select class="input--style-4" name="destination">
                            <%for(ScheduleBean sb:al){%>
                            <option value="<%=sb.getRoute().getDestination()%>"><%=sb.getRoute().getDestination()%></option>
                            <%}%>
                            </select>
                        </div>
                        <div class="input-group">
                            <label class="label">Journey  Date</label>
                            <input class="input--style-4" type="date" name="date" id="userdate" onchange="TDate()" required>
                        </div>
...
function TDate() {
    ...
    if (new Date(UserDate) < ToDate) {
         // alert("The Date must be Bigger or Equal to today date");
          document.getElementById("message").innerHTML = "Date cannot be before today!";
          return false;
     }
     else
     {
